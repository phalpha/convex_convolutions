...
class custom_multiclass_torch(torch.nn.Module):
    ...
    def forward(self, i, setting = "train"):
        ...
        #         self.Z_1_arr_train[key] = torch.matmul(self.Z_1_arr_train[key], self.Z_1_arr_train[key].T) + torch.matmul(self.Z_2_arr_train[key], self.Z_2_arr_train[key].T) + torch.eye(3*f**2, device = device, dtype = dtype)
        #
        #         self.Z_2_arr_train[key] = torch.matmul(self.Z_1_arr_train[key], self.Z_2_arr_train[key]) + torch.matmul(self.Z_2_arr_train[key], self.Z_4_arr_train[key])
        #
        #         self.Z_4_arr_train[key] = torch.matmul(self.Z_4_arr_train[key].T, self.Z_4_arr_train[key]) + torch.matmul(self.Z_4_arr_train[key], self.Z_4_arr_train[key].T) + torch.eye(1, device = device, dtype = dtype)
        #
        #         self.Z_1_arr_prime_train[key] = torch.matmul(self.Z_1_arr_prime_train[key], self.Z_1_arr_prime_train[key].T) + torch.matmul(self.Z_2_arr_prime_train[key], self.Z_2_arr_prime_train[key].T) + torch.eye(3*f**2, device = device, dtype = dtype)
        #
        #         self.Z_2_arr_prime_train[key] = torch.matmul(self.Z_1_arr_prime_train[key], self.Z_2_arr_prime_train[key]) + torch.matmul(self.Z_2_arr_prime_train[key], self.Z_4_arr_prime_train[key])
        #
        #         self.Z_4_arr_prime_train[key] = torch.matmul(self.Z_4_arr_prime_train[key].T, self.Z_4_arr_prime_train[key]) + torch.matmul(self.Z_4_arr_prime_train[key], self.Z_4_arr_prime_train[key].T) + torch.eye(1, device = device, dtype = dtype)
        #
        ...
        for t in range(1,self.C+1):
            ...
            for k in range(1,self.K//self.P+1):
                Z4diff = None
                if convertTo == "positive semidefinite":
                    Z41 = torch.matmul(self.Z_2_arr_train[str(k)+","+str(t)].T, self.Z_2_arr_train[str(k)+","+str(t)]) + torch.matmul(self.Z_4_arr_train[str(k)+","+str(t)], self.Z_4_arr_train[str(k)+","+str(t)].T) + torch.eye(1, device = device, dtype = dtype)
                    Z42 = torch.matmul(self.Z_2_arr_prime_train[str(k)+","+str(t)].T, self.Z_2_arr_prime_train[str(k)+","+str(t)]) + torch.matmul(self.Z_4_arr_prime_train[str(k)+","+str(t)], self.Z_4_arr_prime_train[str(k)+","+str(t)].T) + torch.eye(1, device = device, dtype = dtype)
                    Z4diff = Z41 - Z42
                else:
            ...
            for k in range(1,self.K//self.P+1):
                for l in range(1,self.P+1):
                    Z2diff = None
                    if convertTo == "positive semidefinite":
                        Z21 = torch.matmul(self.Z_1_arr_train[str(k)+","+str(t)], self.Z_2_arr_train[str(k)+","+str(t)]) + torch.matmul(self.Z_2_arr_train[str(k)+","+str(t)], self.Z_4_arr_train[str(k)+","+str(t)])
                        Z22 = torch.matmul(self.Z_1_arr_prime_train[str(k)+","+str(t)], self.Z_2_arr_prime_train[str(k)+","+str(t)]) + torch.matmul(self.Z_2_arr_prime_train[str(k)+","+str(t)], self.Z_4_arr_prime_train[str(k)+","+str(t)])
                        Z2diff = Z21 - Z22
                    else:
            ...
            for k in range(1,self.K//self.P+1):
                for l in range(1,self.P+1):
                    Z1diff = None
                    if convertTo == "positive semidefinite":
                        Z11 = torch.matmul(self.Z_1_arr_train[str(k)+","+str(t)], self.Z_1_arr_train[str(k)+","+str(t)].T) + torch.matmul(self.Z_2_arr_train[str(k)+","+str(t)], self.Z_2_arr_train[str(k)+","+str(t)].T) + torch.eye(3*f**2, device = device, dtype = dtype)
                        Z12 = torch.matmul(self.Z_1_arr_prime_train[str(k)+","+str(t)].T, self.Z_1_arr_prime_train[str(k)+","+str(t)].T) + torch.matmul(self.Z_2_arr_prime_train[str(k)+","+str(t)], self.Z_2_arr_prime_train[str(k)+","+str(t)].T) + torch.eye(3*f**2, device = device, dtype = dtype)
                        Z1diff = Z11 - Z12
                    elif convertTo == "symmetric":
                        Z11 = 0.5 * (self.Z_1_arr_train[str(k)+","+str(t)] + self.Z_1_arr_train[str(k)+","+str(t)].T)
                        Z12 = 0.5 * (self.Z_1_arr_prime_train[str(k)+","+str(t)] + self.Z_1_arr_prime_train[str(k)+","+str(t)].T)
                        Z1diff = Z11 - Z12
                    else:
...
loss_train, accuracies_train, accuracies_val, times = train(model, loss_fn, optimizer, epochs=7)
...
model.eval()  # set model to evaluation mode
with torch.no_grad():
    for t, (x, y) in enumerate(zip(X_test, y_test)):
        scores = model(t, setting = "test")
        current_arr = []
        for i in range(scores.size()[0]):
            current_arr += float(scores[i])
        #current_arr_npy = np.array(current_arr)
        predictions_list.append(current_arr)
        trues_list.append(y)

predictions_list_arr = np.array(predictions_list)
trues_list_arr = np.array(trues_list)

np.save("preds_learningrate{l}.npy".format(l = learn), predictions_list_arr)
np.save("trues_learningrate{l}.npy".format(l = learn), trues_list_arr)
